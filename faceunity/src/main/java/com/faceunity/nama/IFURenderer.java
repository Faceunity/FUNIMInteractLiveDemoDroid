...
abstract class IFURenderer {
    ...
    protected int deviceOrientation = 180;//手机设备朝向
    protected boolean isNeedBufferReturn = true;//单输入是否需要合成后Buffer返回
    protected CameraFacingEnum cameraFacing = CameraFacingEnum.CAMERA_FRONT;  //数据源为相机时候->前后置相机
    protected FUTransformMatrixEnum inputTextureMatrix = FUTransformMatrixEnum.CCROT0_FLIPVERTICAL;//单输入纹理旋转类型
    protected FUTransformMatrixEnum inputBufferMatrix = FUTransformMatrixEnum.CCROT0_FLIPVERTICAL;//单输入图象旋转类型
     ...
     * @return
     */
    public abstract byte[] onDrawFrameDualInput(byte[] img, int texId, int width, int height);
