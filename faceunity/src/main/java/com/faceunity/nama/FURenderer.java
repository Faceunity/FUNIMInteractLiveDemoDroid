...
public class FURenderer extends IFURenderer {
     ...
     */
    @Override
    public byte[] onDrawFrameDualInput(byte[] img, int texId, int width, int height) {
        prepareDrawFrame();
        ...
        config.setNeedBufferReturn(isNeedBufferReturn);
        ...
        if (outputData.getImage().getBuffer() != null) {
    ...
    public byte[] onDrawFrameSingleInput(byte[] img, int width, int height) {
        prepareDrawFrame();
//        inputBufferType = FUInputBufferEnum.FU_FORMAT_I420_BUFFER;
        if (!mRendererSwitch) {
            byte[] black = new byte[img.length];
            Arrays.fill(black, (byte)0x80);
            return black;
        }
        FURenderInputData inputData = new FURenderInputData(width, height);
